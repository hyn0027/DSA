#include <cstring>
#include <cstdio>
const int TABLE_SIZE = 8000023; 
const int overflow_hash_size = 6000011;
...
struct bilateral_square: public collision_strategy {
    int idx;
    void init();
    int bias(int x, int table_size);
    int operator()(hash_entry* Table, int table_size, int last_choice) override;
};

struct public_overflow_area: public collision_strategy {
    void init();
    int operator()(hash_entry* Table, int table_size, int last_choice) override;
};
...
struct hashtable{
    ...
    bool insert(hash_entry entry){ 
        public_overflow_area* pd_collision = dynamic_cast<public_overflow_area*>(my_collision);
        int last_choice = (*my_hashing)(entry.my_string,table_size);
        if (pd_collision) {
            last_choice = (*my_hashing)(entry.my_string, overflow_hash_size);
        }
        my_collision->init();
    ...
    int query(char* query_string){
        ...
        if (pd_collision) {
            last_choice = (*my_hashing)(query_string, overflow_hash_size);
        }
        my_collision->init();
